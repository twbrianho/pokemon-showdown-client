...
(function ($) {
	...
	var User = (this.User = Backbone.Model.extend({
		...
		getActionPHP: function () {
			...
			if (
				location.hostname !== "play.pokemonshowdown.com" &&
				location.hostname !== "pokemonshowdown.com"
			) {
		...
		rename: function (name) {
			...
			if (this.get("userid") !== userid) {
				...
				if (
					...
					// Custom domain -> CORS blocks Smogon login server. Bypass passwords.
					app.send("/trn " + name);
					self.name = name;
					self.named = true;
				} else {
	...
	this.App = Backbone.Router.extend({
		...
		initialize: function () {
			...
			if (this.down) {
				...
				if (
					hostname === Config.routes.client ||
					Config.testclient ||
					hostname.startsWith(Config.defaultserver.id + "-")
				) {
				...
				Storage.whenPrefsLoaded(function () {
					...
					var useHistory =
						!Config.testclient && location.pathname.slice(-5) !== ".html";
		...
		connect: function () {
			...
			var constructSocket = function () {
				if (
					location.host === "localhost.psim.us" ||
					/[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+\.psim\.us/.test(location.host)
				) {
		...
		serializeForm: function (form, checkboxOnOff) {
			...
			var elements = form.querySelectorAll(
				"input[name], select[name], textarea[name], keygen[name], button[value]",
			);
			var out = [];
					...
					Storage.safeJSON(function (data) {
						...
						if (data.actionerror) {
							return app.addPopupMessage(
								"Error loading team: " + data.actionerror,
							);
						}
				...
				if (data === "namerequired") {
					...
					$.ajax(replayLink + ".json", { dataType: "json" })
						.done(function (replay) {
							if (replay) {
								var title =
									replay.players[0] + " vs. " + replay.players[1];
								app.receive(
									">battle-" +
										replayid +
										"\n|init|battle\n|title|" +
			...
			$(document).on("click", "a", function (e) {
				...
				if (
					...
					if (!e.cmdKey && !e.metaKey && !e.ctrlKey) {
						...
						var shortLinks =
							/^(rooms?suggestions?|suggestions?|adminrequests?|forgotpassword|bugs?(reports?)?|formatsuggestions|rules?|faq|credits?|news|privacy|contact|dex|(damage)?calc|insecure|replays?|devdiscord|smogdex|smogcord|forums?|trustworthy\-dlc\-link)$/;
						if (target === "appeal" || target === "appeals")
							target = "view-help-request--appeal";
						if (target === "report") target = "view-help-request--report";
						if (target === "requesthelp")
							target = "view-help-request--other";
		...
		_addRoom: function (id, type, nojoin, title) {
			...
			if (
				["", "teambuilder", "ladder", "rooms", "resources"].indexOf(
					room.id,
				) < 0
			) {
				if (room.isSideRoom) {
			...
			this.$el
				.html(
					'<form><p style="white-space:pre-wrap;word-wrap:break-word">' +
						...
						'</p><p class="buttonbar">' +
						(data.buttons ||
							'<button type="button" name="close" class="button autofocus"><strong>OK</strong></button>') +
						"</p></form>",
				)
	...
	var PromptPopup = (this.PromptPopup = Popup.extend({
		initialize: function (data) {
			...
			buf +=
				...
				'" /></label></p>';
			buf +=
				'<p class="buttonbar"><button type="submit" class="button"><strong>' +
				data.button +
				'</strong></button> <button type="button" name="close" class="button">Cancel</button></p>';
			buf += "</form>";
			...
			update: function (data) {
				...
				if (userid === app.user.get("userid") || !app.user.get("named")) {
					...
					if (userid === app.user.get("userid")) {
						...
						buf +=
							'<button name="login" class="button"><i class="fa fa-pencil"></i> Change name</button> <button name="logout" class="button"><i class="fa fa-power-off"></i> Log out</button>';
					} else {
					...
					buf +=
						'<button name="challenge" class="button">Challenge</button> <button name="pm" class="button">Chat</button> <button name="userOptions" class="button">\u2026</button>';
				}
				buf += "</p>";
	...
	var UserOptionsPopup = (this.UserOptions = Popup.extend({
		...
		update: function () {
			...
			this.$el.html(
				'<p><button name="toggleIgnoreUser" class="button">' +
					ignored +
					"</button></p>" +
					'<p><button name="report" class="button">Report</button></p>' +
					'<p><button name="toggleFriend" class="button">' +
					friended +
					"</button></p>",
			);
		},
		...
		initialize: function (data) {
			...
			if (data.cantconnect) {
				...
				if (window.wiiu && document.location.protocol === "https:") {
					...
					buf +=
						'<p class="buttonbar"><button name="tryhttp" class="button autofocus"><strong>Connect insecurely</button> <button name="close" class="button">Work offline</button></p>';
				} else if (document.location.protocol === "https:") {
					buf +=
						'<p class="buttonbar"><button type="submit" class="button"><strong>Retry</strong></button> <button name="tryhttp" class="button">Retry with HTTP</button> <button name="close" class="button">Work offline</button></p>';
				} else {
					buf +=
						'<p class="buttonbar"><button type="submit" class="button"><strong>Retry</strong></button> <button name="close" class="button">Work offline</button></p>';
				}
			} else if (data.message && data.message !== true) {
				buf += "<p>" + data.message + "</p>";
				buf +=
					'<p class="buttonbar"><button type="submit" class="button autofocus"><strong>Reconnect</strong></button> <button type="button" name="close" class="button">Work offline</button></p>';
			} else {
				buf +=
	...
	this.ProxyPopup = Popup.extend({
		...
		initialize: function (data) {
			...
			buf +=
				'<p class="buttonbar"><button type="submit" class="button"><strong>Submit</strong></button> <button type="button" name="close" class="button">Cancel</button></p>';
			buf += "</form>";
	...
	var ReplayUploadedPopup = (this.ReplayUploadedPopup = Popup.extend({
		...
		initialize: function (data) {
			...
			buf +=
				...
				"/" +
				data.id +
				'</a> <button name="copyReplayLink" class="button">Copy</button></p>';
			buf +=
				'<p><button class="button autofocus" name="close">Close</button><p>';
	...
	var RulesPopup = (this.RulesPopup = Popup.extend({
		...
		initialize: function (data) {
			...
			if (warning) {
				buf +=
					'<p class="buttonbar"><button name="close" disabled class="button">Close</button><small class="overlay-warn"> You will be able to close this in 5 seconds</small></p>';
				setTimeout(_.bind(this.rulesTimeout, this), 5000);
			} else {
				this.type = "semimodal";
				buf +=
					'<p class="buttonbar"><button name="close" class="button autofocus">Close</button></p>';
			}
