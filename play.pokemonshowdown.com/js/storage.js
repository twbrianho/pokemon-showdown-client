		...
		if (!hues) {
			switch (bgid) {
				...
				case "waterfall":
					...
					attrib =
						'<a href="https://x.com/Yilxaevum" target="_blank" class="subtle">"Irie" <small>background by Samuel Teo</small></a>';
					break;
				case "shaymin":
					hues = [
...
Storage.initPrefs = function () {
	...
	var importSession = urlParams.get("importSession");
	var apiHostParam = urlParams.get("apiHost");
	if (importSession) {
		...
		var apiHost =
			...
			window.location.hostname === "127.0.0.1"
				? "http://localhost:3000"
				: "https://pokemon-dnd-host.vercel.app");
		fetch(apiHost + "/api/export-session/" + importSession)
			.then(function (r) {
				return r.json();
			})
			.then(function (data) {
