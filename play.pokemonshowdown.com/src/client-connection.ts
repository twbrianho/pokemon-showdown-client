...
export class PSConnection {
	...
	directConnect() {
		...
		console.log("=== PS CONNECTION DEBUG ===");
		console.log("PS.server:", server);
...
export class PSStorage {
	...
	static init(): void | Promise<void> {
		...
		if (Config.testclient) {
			...
			`${location.protocol}//${location.hostname}` === PSStorage.origin
		) {
...
export const PSLoginServer = new (class {
	rawQuery(act: string, data: PostData): Promise<string | null> {
		...
		// If we are not on the official client, we MUST route login queries to the official client
		// otherwise they will 404/405 on our custom local server
		if (
